...
void MassSelectionFilter::beginJob()
{
  ...
  hBeamlineMassAfterCut  = tfs->make<TH1F>("BeamlineMassAfterCut" ,"BeamLine Mass After Cuts" ,400,-2000,2000);  
  hBeamlineMassBeforeCut = tfs->make<TH1F>("BeamlineMassBeforeCut","BeamLine Mass Before Cuts",400,-2000,2000);  
...
bool MassSelectionFilter::filter(art::Event & evt)
{
  ...
  if(!evt.getByLabel(fTOFModuleLabel,TOFColHandle)) {nFailed++;return false;}
  ...
  if (radical<0){radical= -radical;}
  float mass = reco_momo*pow(radical ,0.5);
  if((reco_tof*0.299792458*0.299792458*reco_tof/(fDistanceTraveled*fDistanceTraveled) - 1)<0){mass=-mass;}
 
  if(std::isnan(mass)) {nFailed++;return false;}

...
void AGCounterAlg::ImportWaveform(std::string AuxDetName, std::vector<raw::AuxDetDigit> Digits) {
	if (AuxDetName == "USE") {
		fUSEDigits = Digits;
...
std::vector<int> AGCounterAlg::HitsFinder(raw::AuxDetDigit WaveformDigit) {
	float Threshold = -40;																	// Check Hardcoded! Hit Threshold
	...
	for (size_t i = 2; i < WaveformDigit.NADC()-2; ++i) {
		...
		float Derivative = float(8*(WaveformDigit.ADC(i+1))-8*(WaveformDigit.ADC(i-1))+WaveformDigit.ADC(i-2)-WaveformDigit.ADC(i+2))/12;
...
std::vector<std::vector<int> > AGCounterAlg::HitsTimeMatching(std::vector<int> USEHits, std::vector<int> USWHits, std::vector<int> DS1Hits, std::vector<int> DS2Hits) {
		...
		for (size_t j = 0; j < USWHits.size(); j++) {
			for (size_t k = 0; k < DS1Hits.size(); k++) {
				for (size_t l = 0; l < DS2Hits.size(); l++) {
					HitTimePairing.push_back(USEHits.at(i));
...
bool AGCounterAlg::CheckMatched(std::vector<int> HitTimePairing) {
	short TimeThreshold = 15; // Check Hardcoded! Hit Time Seperation
	bool FormedHitGroup;
	std::vector<int> TimeMetrics;
	
	for (size_t i = 0; i < HitTimePairing.size(); i++) {
		if (HitTimePairing.at(i) != 0) {
			TimeMetrics.push_back(HitTimePairing.at(i));
		}
	}
	...
	for (size_t i = 0; i < TimeMetrics.size(); i++) {
		for (size_t j = i+1; j < TimeMetrics.size(); j++) {
			if (std::abs(TimeMetrics.at(i)-TimeMetrics.at(j)) > TimeThreshold) {
...
float AGCounterAlg::PulseAreaFinder(raw::AuxDetDigit Digit, long unsigned int HitTime) {
...
std::vector<std::vector<AGCHits> > AGCounterAlg::AGCHitsWrapper() {
	std::vector<AGCHits>		   AllHitsGroup;
	AGCHits				   SingleHit;
	
	for (size_t i = 0; i < fUSEDigits.size(); i++) {
		std::vector<int> USEHits = HitsFinder(fUSEDigits.at(i));
		std::vector<int> USWHits = HitsFinder(fUSWDigits.at(i));
		std::vector<int> DS1Hits = HitsFinder(fDS1Digits.at(i));
		std::vector<int> DS2Hits = HitsFinder(fDS2Digits.at(i));
		...
		for (size_t j = 0; j < AllHitsTimePairing.size(); j++) {
			SingleHit.TriggerTimeStamp = fUSEDigits.at(i).TimeStamp();
			...
			if (AllHitsTimePairing.at(j).at(0) == 0) { SingleHit.HitExistUSE = false; SingleHit.HitPulseAreaUSE = 0.; }
			if (AllHitsTimePairing.at(j).at(1) == 0) { SingleHit.HitExistUSW = false; SingleHit.HitPulseAreaUSW = 0.; }
			if (AllHitsTimePairing.at(j).at(2) == 0) { SingleHit.HitExistDS1 = false; SingleHit.HitPulseAreaDS1 = 0.; }
			if (AllHitsTimePairing.at(j).at(3) == 0) { SingleHit.HitExistDS2 = false; SingleHit.HitPulseAreaDS2 = 0.; }
			
			if (SingleHit.HitExistUSE == true) { SingleHit.HitPulseAreaUSE = PulseAreaFinder(fUSEDigits.at(i), SingleHit.HitTimeStampUSE); }
			if (SingleHit.HitExistUSW == true) { SingleHit.HitPulseAreaUSW = PulseAreaFinder(fUSWDigits.at(i), SingleHit.HitTimeStampUSW); }
			if (SingleHit.HitExistDS1 == true) { SingleHit.HitPulseAreaDS1 = PulseAreaFinder(fDS1Digits.at(i), SingleHit.HitTimeStampDS1); }
			if (SingleHit.HitExistDS2 == true) { SingleHit.HitPulseAreaDS2 = PulseAreaFinder(fDS2Digits.at(i), SingleHit.HitTimeStampDS2); }

...
namespace wct {
  ...
  void WCTrackAnalyzer::beginJob()
  {
    ...
    // is drawn.
    fPHist           = tfs->make<TH1D>("momenetum", "Momentum of WC Tracks", fPBins, fPLo, fPHi);

    fXFaceHist       = tfs->make<TH1D>("X_Face","X Location of Track's TPC Entry (mm)", fXBins, fXLo, fXHi);

    fYFaceHist       = tfs->make<TH1D>("Y_Face","Y Location of Track's TPC Entry (mm)", fYBins, fYLo, fYHi);    

    fXWireHist       = tfs->make<TH1D>("xwire", "X Wire Values",fWCScale*fXBins, fWCScale*fXLo, fWCScale*fXHi);

    fYWireHist       = tfs->make<TH1D>("ywire", "Y Wire Values",fWCScale*fYBins, fWCScale*fYLo, fWCScale*fYHi);

    fXAxisHist       = tfs->make<TH1D>("xaxis", "X Axis Values", fXBins, fXLo, fXHi);
   
    fYAxisHist       = tfs->make<TH1D>("yaxis", "Y Axis Values", fYBins, fYLo, fYHi);

    fY_Kink          = tfs->make<TH1D>("Y_Kink","Angle between US/DS tracks in Y direction (degrees)",fKinkBins,fKinkLo*3.1415926/180,fKinkHi*3.141592654/180);
    ...
    fNTracks         = tfs->make<TH1D>("ntracks", ";Number of WC Tracks in Events;", fNBins, fNLo, fNHi);

    fWCTDCs          = tfs->make<TH1D>("wctdcs", ";TDC Counts from Wire Chambers;", fTimeBins, fTimeLo, fTimeHi);

    //Enter 2D Histograms
    fXYFaceHist      = tfs->make<TH2D>("xyface", "XY Positions on TPC Face", fXBins, fXLo, fXHi, fYBins, fYLo, fYHi);
    ...
    for(size_t wcIt = 0; wcIt < fNWC; ++wcIt)
    {
      ...
      TH2D* wireHist = tfs->make<TH2D>(("xywire"+wcName.str()).c_str(), ("XY Wire Values" + wcSs.str()).c_str()
                                            ,fWCScale*fXBins, fWCScale*fXLo, fWCScale*fXHi
                                            , fWCScale*fYBins, fWCScale*fYLo, fWCScale*fYHi);

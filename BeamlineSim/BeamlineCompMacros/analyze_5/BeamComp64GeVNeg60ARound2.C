#define BeamComp64GeVNeg60ARound2_cxx
#include "BeamComp64GeVNeg60ARound2.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <TRandom3.h>
...
void BeamComp64GeVNeg60ARound2::Loop()
{
//Get the data plots
//This could be cleaned up once we have a "production" of beamline reco and a standardized file system.
TFile *fp=new TFile("../FinalBeamComp/WCTrackHistosNeg60APicky.root"); //Change this your data picky tracks after WCFilter
TH1F *dp=(TH1F*)fp->Get("FourPointMom");
TFile *fh=new TFile("../FinalBeamComp/JHoWCTrackHistosNeg60.root");//Change this to your data 3 pt tracks after WCFilt 
TFile *fhy=new TFile("../FinalBeamComp/Data60Plots/Neg60.root");//File With the TOF and HY tracks
...
//Now format some fills colors and lines.
   //Picky plots with a bad looking color scheme
   pip->SetLineColor(9);
   mup->SetLineColor(41);
   ep->SetLineColor(15);
   kp->SetLineColor(7);
...
/*   //Make the S3 Point Momentum Comparion Plot       
   ...
   gr4->SetTitle("Pos 100A HY 4 Point: Bin by Bin Fractional Error Between Sim and Data");
...
THStack *h4c=new THStack("Corrected Four Point HY Beam Comp 64GeV Pos 100A","Corrected Four Point HY Beam Comp 64GeV Pos 100A");
...
//Prepare the mass plot color scheme
   ...
   h4m->Add(p4m,"hist");
   h4m->Add(pi4m,"hist");
      //Make the Hy 4 Point Mass Comparison Plot, using the smeared TOF/mom   
   ...
   leg4m->SetHeader("64 GeV +60 Amps");
   ...
   leg4t->SetHeader("64 GeV +60 Amps");
  ...
  //Make the Bin by Bin Content Plot    
   ...
   leg4cont->SetHeader("64 GeV +60 Amps");
...
double BeamComp64GeVNeg60ARound2::FindPDG(double(&pdg)[4][2], double (&mother)[4][2], double (&id)[4][2])
{
  double pdg1=pdg[3][0];
  double pdg2=pdg[3][1];
  double id1=id[3][0];

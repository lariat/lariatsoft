...
//
// SimWireLArIAT class designed to simulate signal on a wire in the TPC
//
// brebel@fnal.gov
//
//
////////////////////////////////////////////////////////////////////////
#ifndef SIMWIRELARIAT_H
#define SIMWIRELARIAT_H
...
namespace detsim {
  ...
  class SimWireLArIAT : public art::EDProducer {
    
  public:
        
    explicit SimWireLArIAT(fhicl::ParameterSet const& pset); 
    virtual ~SimWireLArIAT();
...
namespace detsim{
  ...
  //-------------------------------------------------
  SimWireLArIAT::SimWireLArIAT(fhicl::ParameterSet const& pset)
  {
  ...
  //-------------------------------------------------
  void SimWireLArIAT::reconfigure(fhicl::ParameterSet const& p) 
  {
    fResponseSet      = false;
    cet::search_path sp("FW_SEARCH_PATH");
    if( !sp.find_file(p.get<std::string>("ResponseFile"), fResponseFile) )
      throw cet::exception("SimWireLArIAT") << "Unable to find electronics response file in\n"
					  << sp.to_string()
  ...
  //-----------------------------------------------
   void SimWireLArIAT::beginRun(art::Run& run) 
  { 
  ...
  //-------------------------------------------------
  void SimWireLArIAT::endJob() 
  {
  }

  //-------------------------------------------------
  void SimWireLArIAT::produce(art::Event& evt)
  {
    //std::cout << "in SimWireLArIAT::produce " << std::endl;
  ...
  //-------------------------------------------------
  void SimWireLArIAT::ConvoluteResponseFunctions()
  {
  ...
  //-------------------------------------------------
  void SimWireLArIAT::GenNoise(std::vector<float>& noise)
  {
    art::ServiceHandle<art::RandomNumberGenerator> rng;
    CLHEP::HepRandomEngine &engine = rng->getEngine();
    CLHEP::RandFlat flat(engine);
  ...
  //-------------------------------------------------
  void SimWireLArIAT::SetFieldResponse()
  {
  ...
  //-------------------------------------------------
  void SimWireLArIAT::SetElectResponse()
  {
    ...
    if(inFile.IsZombie()){      
      throw cet::exception("SimWireLArIAT") << "Cannot open response file" << fResponseFile.c_str();
    }    
...
namespace detsim{
  ...
  DEFINE_ART_MODULE(SimWireLArIAT)

}

#endif // SIMWIRELARIAT_H

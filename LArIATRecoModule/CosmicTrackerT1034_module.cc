  ...
  void CosmicTrackerT1034::produce(art::Event& evt){
    ...
    for(size_t t = 0; t < tdu.NTriggers(); ++t)        // Loop over triggers                          
    {
       ...
       for (size_t itrk = 0; itrk<matchedclusters.size(); ++itrk)    //loop over tracks
       {
          ...
          }//iclu Loop over clusters
          
          // Remove isolated hits
          for (size_t iclu = 0; iclu<vtimemap.size(); ++iclu)   // Loop over cluster
          {
          ...
          }   // loop over cluster to remove isolated hits
          ...
          for (size_t iclu = 0; iclu<vtimemap.size(); ++iclu)   //loop over clusters
          {
...
//             std::cout<<timee2->first<<" "<<timee2->second<<std::endl;
	     ...
	     auto besthit2 = vhitmap[iclu2].end();
             ...
             if(spacepoints.size()>0)
             {
                ...
                {
                   for (int s = 0; s < int(xyz.size()); ++s) // loop over spacepoints
                   {
                      int np = 0;
		...
		tcol->push_back(recob::Track(recob::TrackTrajectory(recob::tracking::convertCollToPoint(xyz),
	        ...
	        util::CreateAssn(*this, evt, *tcol, trig, *TrigTrackAssn);
               ...
               if (!fdebug)
               {
                  ...
                  {
                     util::CreateAssn(*this, evt, *tcol, fm.at(TrackClusters[cpt]), *thassn);
